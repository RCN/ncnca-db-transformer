...
const getSeries = (conn, eventId) => conn.query(`
  ...
  Promise.all([getEvents(conn), /*getPromoters(conn)*/])
    .then(([eventsRaw, /*promotersRaw*/]) => {
      ...
      return Promise.map(events,
        event => {
          const fetchOtherEventData =
            getPromotersForEvent(conn, event._id)
            ...
            .then(() => getFlyersForEvent(conn, event._id))
              .then(flyersRaw => {
                const flyer = _.first(flyersRaw)
                return {
                  ...event,
                  // flyerName: event.flyerRelativePath + event.flyerName
                  flyer: {
                    name: flyer.flyerName,
                    url: parser.getFlyerUrl(flyer.flyerRelativePath), //relative path includes name
                    mimeType: flyer.flyerMimeType
                  },
                  flyerUrl: parser.getFlyerUrl(flyer.flyerRelativePath), //relative path includes name
                  usacPermit: parser.getUsacPermit(event.permitNumber, event.resultsUrl, flyer.flyerName),
                }
              })
            .then(() => getSeries(conn, event._id))
            .then(seriesRaw => {
              return {
                ...event,
          ...
          return fetchOtherEventData
          // return Promise.all([getPromoters])
        },
        {concurrency: 10}
      )
    })
    .then(events => {
      ...
      // ])
        ...
        const postProcessedEvents = Object.keys(groupedByIdEvents)
          ...
          return mapToNewIds(processedEvents)
        })
