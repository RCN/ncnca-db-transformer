...
.then(rows => {
  ...
  const toSimpleEvent = event => ({
    ...
    usacPermit: parser.getUsacPermit(event.permitNumber, event.resultsUrl),
    resultsUrl: parser.getRegUrl(event.resultsUrl)
  })
  ...
  const eventsForPrint = Object.keys(groupedEvents).map(year => {
    ...
    return {
      [year]: _.uniq((groupedEvents[year].map(x => `${x.resultsUrl}`))).length,
      x: _.uniq((groupedEvents[year].map(x => `${x.resultsUrl}`))),
