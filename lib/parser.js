...
const getType = (rawType, rawName) => {
  const EventTypeMap = {
    'CYCLOCROSS': 'Cyclocross',
    'MTB': 'MTB',
    'TRACK': 'Track',
    'CLINICS': 'Clinics',
    'NCNCA MEETING': 'Meeting',
  ...
  //sometimes NCNCA meetings are marked with all possible event types, I guess that
  //was done so they show up in the calendar irrespective of selected filter
  const eventType = (rawName.toLowerCase().includes('meeting') && EventTypeMap[rawType] !== 'Meeting')
   ? 'Meeting'
   : EventTypeMap[rawType]
...
const getDiscipline = (rawType, rawName) => {
  ...
  //sometimes NCNCA meetings are marked with all possible event types, I guess that
  //was done so they show up in the calendar irrespective of selected filter
  const eventType = (rawName.toLowerCase().includes('meeting') && EventTypeToDisciplineMap[rawType] !== 'Other')
   ? 'Other'
   : EventTypeToDisciplineMap[rawType]
...
const canceledEventRegex = /([ -]*\(?cancell?ed\)?[ -]*(road closure)?)/i
